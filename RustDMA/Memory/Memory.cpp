...
bool Memory::DumpMemoryMap(bool debug)
{
	...
	LPCSTR args2[7];
	for (int i = 0; i < 7; ++i) {
		args2[i] = static_cast<LPCSTR>(args[i]);
	}

	VMM_HANDLE handle = VMMDLL_Initialize(argc, args2);
	if (!handle)
	{
...
bool Memory::Init(std::string process_name, bool memMap, bool debug)
{
	...
	reinit:
		...
		this->vHandle = VMMDLL_Initialize(argc, args2);
		if (!this->vHandle)
		{
			if (memMap)
			{
...
std::string Memory::ReadString(uintptr_t address, size_t length) const
{
